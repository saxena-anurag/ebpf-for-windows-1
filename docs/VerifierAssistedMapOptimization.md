...
|----------|------|------|--------|
| **Forward r1 register tracking** in bpf2c | Simple, no external changes | Misses maps loaded across branches; fragile if instruction patterns change | Rejected |
| **Verifier CFG instruction scanning** | No private API access needed | Map fd numbering mismatch between verifier and bpf2c; not control-flow aware | Rejected |
| **Verifier abstract domain** (chosen) | Semantically correct for all control-flow patterns; proven by the verifier | Requires making PREVAIL methods public; requires map name bridging | **Implemented** |

### Why the verifier approach is correct
